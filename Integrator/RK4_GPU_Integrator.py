...
def comp_k1_term(init_conds, dydt, step, dt, k1):
    ...
    for i in range(len(init_conds)):
          k1[i] = dt*dydt[i] # or better multiply by dt in final equation?
...
def comp_k2_term(init_conds, dydt, step, dt, k1, k2, buffer): 
    ...
    func(buffer, dydt, t_)  
    for i in range(len(init_conds)):
        k2[i] = dt*dydt[i] # or better multiply by dt in final equation?
...
def comp_k3_term(init_conds, dydt, step, dt, k2, k3, buffer):
    ...
    func(buffer, dydt, dt)
    for i in range(len(init_conds)):
          k3[i] = dt*dydt[i] # or better multiply by dt in final equation?
...
def comp_k4_term(init_conds, dydt, step, dt, k3, k4, buffer):
    ...
    for i in range(len(init_conds)):
          k4[i] = dt*dydt[i] # or better multiply by dt in final equation?


@cuda.jit # ('void(float32[:], float32[:], float32, int16)')
def RK4_sol_cuda(init_conds, dt, steps, k1, k2, k3, k4, buffer): # for intermediate calc-ns 
    ...
    for i in range(start, init_conds.shape[0], stride): 
        ...
        for step in range(1, steps):   # so ?         
            ...
            #cuda.syncthreads() # Is it in the right place?
            comp_k2_term(init_conds[i,:], step, dt, k1[i,:], k2[i,:], buffer[i,:])
            #cuda.syncthreads() # Is it in the right place? 
            comp_k3_term(init_conds[i,:], step, dt, k2[i,:], k3[i,:], buffer[i,:])
            #cuda.syncthreads() # Is it in the right place? 
            comp_k4_term(init_conds[i,:], step, dt, k3[i,:], k4[i,:], buffer[i,:])
            #cuda.syncthreads() # Is it in the right place?             
